...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        nearest_p = self.place
        dist = 0
        while dist <= self.max_range:
            #if nearest_p.is_hive == True:     为什么不行？
            while dist < self.min_range:
                nearest_p = nearest_p.entrance
                dist += 1
                if nearest_p is beehive:
                    return None
            if nearest_p is beehive:
                return None
            if nearest_p.bees:
                return bee_selector(nearest_p.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health > amount:
            for bee in self.place.bees[:]: #a为列表，对其中对象更改，若是更改其属性，使b=a[:],对b遍历，也可更改a中每个对
                                           #象实例同时防止a中更改属性使对象消失后，漏掉对消失对象后的下一个对象的属性进行更改
                Ant.reduce_health(bee, amount) 
        else:
            for bee in self.place.bees[:]:
                Ant.reduce_health(bee, amount + self.damage)
        Ant.reduce_health(self, amount) #反弹伤害后再减生命，生命为0失去现有place属性，无法追踪在这个place中的bees属性
    ...
    def action(self, gamestate):
        if self.chewing > 0:
            ...
            if self.place.bees:
                ...
                selector = bee_selector(self.place.bees)
                Bee.reduce_health(selector, selector.health)
# END Problem 7
...
# END Problem EC
        ...
        if self.queen:
            ...
            while curr_place != None:
                if curr_place.ant != None:
                    if curr_place.ant.buffed == 0:
                        curr_place.ant.buff()
                        curr_place.ant.buffed = 1
                    if curr_place.ant.is_container() == True:
                        if curr_place.ant.contained_ant != None and curr_place.ant.contained_ant.buffed == 0:
                            curr_place.ant.contained_ant.buff()
                            curr_place.ant.contained_ant.buffed = 1
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and self.queen:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_flag:
            self.slow_turns = self.slow_turns - 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target:
            target.scare(2) 
            '''if target.scare_flag == False:
                target.scare_flag = True'''
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        iif = {}
        dist = 0
        if curr_place.ant is not self:
            iif[curr_place.ant] = dist
        if curr_place.bees != []:
        ...
        while curr_place is not beehive:
            if curr_place.ant is not None:
                ...
                if curr_place.ant.is_container():
                    if curr_place.ant.contained_ant is not None:
                        iif[curr_place.ant.contained_ant] = dist
            if curr_place.bees != []:
                for bee in curr_place.bees:
                    iif[bee] = dist
            dist = dist + 1
