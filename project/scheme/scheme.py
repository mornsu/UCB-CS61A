...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        python_args = python_args + helper(args)
        if self.use_env:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals, args)#需要用self.env创建而非env
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #target = scheme_eval(expressions.rest, env) 错误，若是(define size 2)，expressions.rest为Pair(2, nil)，这是一个只包含一个元素 2 的列表。scheme_eval 发现 (2) 是一个列表，并且第一个元素 2 不是一个特殊形式的关键字。
