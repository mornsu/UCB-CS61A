...
def swine_align(player_score, opponent_score):
    ...
    if player_score-opponent_score<0:
        ...
        small = opponent_score
    GCD = 10
    while GCD<=small:
        if player_score%GCD == 0:
            if opponent_score%GCD == 0:
                return True
        GCD = GCD+1
    return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # (note that the indentation for the problem 6 prompt (***YOUR CODE HERE***) might be misleading)
        ...
        "*** YOUR CODE HERE ***"
        if n == 0:
            #talk = both(say_scores,announce_lead_changes())
            #talk = talk(score0,score1)
            talk = say
            talk = talk(score0,score1)
            n = 1
        else:
    ...
    def say(score0,score1):
        if who == 0:
            ...
            if running_high < score0-last_score:
                ...
                print(highest,"point(s)! The most yet for Player",who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num < 11:
        average = averaged_dice(num,dice)
        if average>max:
            max = average
            max_num = num
        num = num+1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if add>=cutoff:
        return 0
    flag = extra_turn(score+add,opponent_score)
    if flag:
        return 0
    return num_rolls  # Replace this statement
    # END PROBLEM 11
