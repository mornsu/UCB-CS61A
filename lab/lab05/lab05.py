...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        new_t = tree(label(t), [tree(l) for l in leaves] )
        return new_t
    else:
        new_b = [ sprout_leaves(b, leaves) for b in branches(t)]
        new_t = tree(label(t), new_b)
    return new_t
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [ deck[n//2] if n%2==0 else deck[ (len(deck)+n-1)//2  ] for n in range(len(deck)) ]
...
def add_trees(t1, t2):
    ...
    re_label = label(t1)+label(t2)
    re_branches = []
    i = 0
    while i < min(len(branches(t1)), len(branches(t2))):
        b1, b2 = branches(t1)[i], branches(t2)[i]
        new_branch = add_trees(b1,b2)
        re_branches = re_branches + [new_branch]
        i += 1
    re_branches = re_branches + branches(t1)[i:] + branches(t2)[i:]
    return tree(re_label, re_branches)
