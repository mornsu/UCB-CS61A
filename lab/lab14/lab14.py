...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    b0 = t.branches[0].label
    b1 = t.branches[1].label
    if b0 > b1:
        t = [t.branches[1]]
    else:
...
def num_splits(s, d):
    ...
    def helper(s, l1, l2):
        if s == []:
            result = abs(sum(l1) - sum(l2))
            if result > d:
                return 0
            else:
                return 1
        else:
            return helper(s[1:], l1 + [s[0]], l2) + helper(s[1:], l1, l2 + [s[0]])
    return int(helper(s, [], [])/2)
