...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.current_year <= self.year:
            return self.cents
        w = self.cents + Mint.current_year - self.year - 50
        return w
...
def is_bst(t):
    ...
    if num == 1:
        b1 = t.branches[0]
        if bst_max(b1) > t.label and bst_min(b1) < t.label:
            return False
        return is_bst(b1)
    b1 = t.branches[0]
    b2 = t.branches[1]
    if bst_max(b1) > t.label:
        return False
    if bst_min(b2) < t.label:
        return False
